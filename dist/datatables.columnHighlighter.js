/*!
 HTMLExtensions v0.1.8 — DataTables ColumnHighlighter & ToggleView
 (c) 2011–2025 Przemyslaw Klys @ Evotec
 https://htmlextensions.evotec.xyz | MIT License | Build: 2025-10-01T21:34:50.472Z
*/
...
(function () {
  ...
  function dataTablesCheckCondition(condition, data) {
    ...
    if (condition['type'] == 'bool') {
      ...
      var momentConversion =
        typeof moment !== 'undefined' ? moment(columnValue, condition['dateTimeFormat']) : columnValue;
    ...
    if (operator == 'eq') {
      ...
      return Array.isArray(right) && right.indexOf(left) != -1;
    } else if (operator == 'notin') {
      return Array.isArray(right) && right.indexOf(left) == -1;
    } else if (operator == 'contains' || operator == 'like') {
      var rx = new RegExp(right);
      return rx.test(left);
    } else if (operator == 'notcontains' || operator == 'notlike') {
      var rx2 = new RegExp(right);
      return !rx2.test(left);
  ...
  function headerNameToIndex(tableId, name) {
    ...
    for (var i = 0; i < headers.length; i++) {
      if (('' + headers[i]).toLowerCase() === lname) return i;
    }
    for (var j = 0; j < headers.length; j++) {
      if (normalizeName(headers[j]) === nname) return j;
    }
    return -1;
  }
  ...
  window.DataTablesColumnHighlighter = {
    ...
    detectStore: function (table) {
      try {
        if (table && table.rows) {
          var d = table.rows({ page: 'current' }).data();
          var f = d && d.length ? d[0] : null;
          if (Array.isArray(f)) return 'HTML';
          if (f && typeof f === 'object') return 'JavaScript';
        }
      } catch (e) {}
      return 'HTML';
    },
      ...
      try {
        ...
        if ((c.columnId === undefined || c.columnId === -1) && c.columnName) {
          ...
          for (var h = 0; h < headers.length; h++) {
            if ((headers[h] + '').toLowerCase() === lname) {
              c.columnId = h;
              break;
            }
          }
          if (c.columnId === undefined || c.columnId === -1) {
            var nname = normalizeName(c.columnName);
            for (var h2 = 0; h2 < headers.length; h2++) {
              if (normalizeName(headers[h2]) === nname) {
                c.columnId = h2;
                break;
              }
            }
        ...
        if ((!c.columnName || c.columnName === '') && typeof c.columnId === 'number') {
          if (c.columnId >= 0 && c.columnId < headers.length) {
            c.columnName = headers[c.columnId];
          }
        }
      } catch (e) {}
      return c;
    ...
    normalizeRules: function (tableId, config, table) {
      try {
        ...
        for (var i = 0; i < config.length; i++) {
          var rule = config[i];
          if (rule && rule.conditionsContainer) {
            for (var r = 0; r < rule.conditionsContainer.length; r++) {
              var container = rule.conditionsContainer[r];
              if (container && container.conditions) {
                for (var k = 0; k < container.conditions.length; k++) {
                  container.conditions[k] = this.normalizeCondition(container.conditions[k], headers, store);
                }
              }
            }
          }
    ...
    setupEventHandlers: function (tableId, table) {
        ...
        table.on('responsive-display', function (e, datatable, row, showHide) {
          try {
            if (showHide) {
              ...
              var $tr = jQuery(tr);
              var data = row.data();
              self.applyHighlighting(tableId, $tr, data);
            }
          } catch (err) {}
        });
      ...
      if (!(table && table.on)) {
        jQuery('#' + tableId + ' tbody').on(
          'click',
          'td.dtr-control, td.dt-control, .dtr-control, .dt-control',
          function () {
            var $tr = jQuery(this).closest('tr');
            setTimeout(function () {
              var childRow = $tr.next('tr.child');
              if (childRow.length > 0) {
                var rowData = [];
                $tr.find('td').each(function () {
                  var $td = jQuery(this);
                  if (!$td.hasClass('dtr-control') && !$td.hasClass('dt-control')) {
    ...
    applyHighlighting: function (tableId, $parentRow, rowData) {
      var cfg = this.configurations[tableId];
      if (!cfg) {
        return;
      }
      var childRow = $parentRow.next('tr.child');
      var norm = this.normalizeRowData(rowData);
      var styleMap = {};
      function pushStyle(col, style, isPass) {
        if (!col) return;
        if (!styleMap[col]) styleMap[col] = { pass: [], fail: [] };
        (isPass ? styleMap[col].pass : styleMap[col].fail).push(style || {});
      }
      function normalizeTarget(t) {
        return {
          ...
          css: t.css,
          highlightParent: t.highlightParent,
        };
      }
      function mergedStyle(entry) {
        var src = entry.pass.length > 0 ? entry.pass : entry.fail;
        ...
        for (var i = 0; i < src.length; i++) {
          ...
          if (s.highlightParent) out.highlightParent = s.highlightParent;
          if (s.css) {
            out.css = out.css || {};
            for (var k in s.css) {
              if (Object.prototype.hasOwnProperty.call(s.css, k)) out.css[k] = s.css[k];
            }
          }
        }
        return out;
      ...
      for (var r = 0; r < cfg.config.length; r++) {
        ...
        var matched = rule.conditionsContainer
          ? (function () {
              try {
                var found = false,
                  cm = [];
                for (var i = 0; i < rule.conditionsContainer.length; i++) {
                  var container = rule.conditionsContainer[i];
                  cm = [];
                  for (var k = 0; k < container['conditions'].length; k++) {
                    var condition = container['conditions'][k];
                    cm.push(dataTablesCheckCondition(condition, norm));
                  }
        ...
        if (matched) {
          for (var t = 0; t < rule.targets.length; t++) {
            var nt = normalizeTarget(rule.targets[t]);
            pushStyle(nt.column, nt, true);
          }
        } else if (rule.failTargets) {
          for (var ft = 0; ft < rule.failTargets.length; ft++) {
            var nf = normalizeTarget(rule.failTargets[ft]);
            pushStyle(nf.column, nf, false);
          }
        }
      }
      for (var col in styleMap) {
        if (!Object.prototype.hasOwnProperty.call(styleMap, col)) continue;
        var style = mergedStyle(styleMap[col]);
        if (!style) continue;
        ...
        var target = {
          ...
          css: style.css,
          highlightParent: style.highlightParent,
        };
        if (childRow.length > 0) {
          DataTablesColumnHighlighter.applyColumnStyling(childRow, target);
        }
    ...
    applyVisibleCellStyling: function (tableId, $parentRow, target) {
      try {
        var idx =
          typeof target.column === 'number' ? target.column : headerNameToIndex(tableId, target.column);
        if (idx < 0) return;
        ...
        if ($cell && $cell.length > 0) {
          ...
          if (target.textColor) {
            $cell.css('color', target.textColor);
          }
          ...
          if (target.backgroundColor) {
            ...
            try {
              $cell.css('--bs-table-accent-bg', 'transparent');
            } catch (e1) {
              /* noop */
            }
    ...
    createConfig: function (conditions, targets) {
      return [{ condition: conditions, targets: Array.isArray(targets) ? targets : [targets] }];
    },
  };

  window.setupChildRowConditionalFormatting = function (
    tableId,
    conditionsContainer,
    highlightColumn,
    css,
    ...
    try {
      var headers = getHeaderNames(tableId);
      var detectStore = function () {
        try {
          if (table && table.rows) {
            var d = table.rows({ page: 'current' }).data();
            var first = d && d.length ? d[0] : null;
            if (Array.isArray(first)) return 'HTML';
            if (first && typeof first === 'object') return 'JavaScript';
          }
        } catch (e) {}
        return 'HTML';
      };
      ...
      try {
        for (var i = 0; i < conditionsContainer.length; i++) {
          ...
          if (!container || !container.conditions) continue;
          for (var k = 0; k < container.conditions.length; k++) {
            var c = container.conditions[k] || {};
            if (!c.dataStore) {
              c.dataStore = dataStoreType;
            }
            if ((c.columnId === undefined || c.columnId === -1) && c.columnName) {
              var lname = ('' + c.columnName).toLowerCase();
              for (var h = 0; h < headers.length; h++) {
      ...
      if (Array.isArray(highlightColumn)) {
        for (var i = 0; i < highlightColumn.length; i++) {
          var entry = highlightColumn[i];
          var header = null;
          if (typeof entry === 'number') {
            header = indexToHeaderName(tableId, entry);
          } else if (typeof entry === 'string') {
            header = entry;
          } else if (entry && typeof entry === 'object' && entry.column) {
            header = entry.column;
          }
          if (header !== null && header !== undefined) {
            var t = { column: header };
            if (css) {
              if (css['background-color']) t.backgroundColor = css['background-color'];
              if (css['color']) t.textColor = css['color'];
              t.css = css;
            }
            targets.push(t);
          }
      ...
      if (
        window.DataTablesColumnHighlighter &&
        window.DataTablesColumnHighlighter.configurations &&
        window.DataTablesColumnHighlighter.configurations[tableId]
      ) {
        try {
          window.DataTablesColumnHighlighter.configurations[tableId].config.push(newRule);
          var tbl = table || window.DataTablesColumnHighlighter.configurations[tableId].table;
          if (tbl && tbl.rows) {
            tbl.rows({ page: 'current' }).every(function () {
              var tr = this.node();
              var $tr = jQuery(tr);
              var data = this.data();
              window.DataTablesColumnHighlighter.applyHighlighting(tableId, $tr, data);
            });
          }
        } catch (e) {
          window.DataTablesColumnHighlighter.init(
            tableId,
            [newRule],
            table || window.DataTablesColumnHighlighter.configurations[tableId].table
          );
        }
      } else {
        window.DataTablesColumnHighlighter.init(tableId, [newRule], table);
      }
    } catch (e) {}
  };
  ...
  function autoInitFromSettings(settings) {
    try {
      ...
      var ch = oInit.columnHighlighter;
      if (!ch) return;
      var rules = ch.rules || ch.config || [];
